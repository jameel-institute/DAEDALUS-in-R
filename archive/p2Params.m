...
nnh     = nn.*dis.ihr;
nnhtot  = [nnh(1),sum(nnh(2:4)),sum(nnh(5:13)),sum(nnh(14:end))]; %1*17
...
%Transmission
Deff  = Dout.*repmat(data.NNs,1,ntot)./repmat(data.NNs',ntot,1); % [49 x 49]
onesn = ones(ntot,1);
F     = zeros(3*ntot,3*ntot); 
F(1:ntot,ntot+1:end)=[dis.red*Deff,Deff];   % [147 x 147] 
...
GD=F/V; % [147 x 147]
d=eigs(GD,1);%largest in magnitude (+/-)  % 7.29
R0a=max(d); % 7.29
...
%lgh = zeros(length(lg), length(lg));

% lgh = discounted life expectancy for each age group
% discounting = cost that occurs in the future is not as significant (not as large) in the
% present
